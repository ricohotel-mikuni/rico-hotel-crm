  ...
  return (
    <div style={{ padding:'18px 16px', maxWidth:1000, margin:'0 auto' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
        <div>
          <h1 style={{ fontSize:16, fontWeight:700, color:C.navy, margin:0 }}>案件管理</h1>
          <div style={{ fontSize:11, color:'#90A4AE' }}>全 {cases.length} 件 / 成約売上: {fmt(wonRev)}円</div>
        </div>
      ...
      {modal && (
        <Modal title="案件情報" icon="ti-clipboard-list" onClose={()=>setModal(false)} onSave={save} saving={saving} width={540}>
          ...
          <G2>
            <FS label="ステータス" value={form.status} onChange={set('status')} options={CASE_STATUS} />
            <FI label="成約確率（%）" value={form.probability} onChange={v=>set('probability')(Number(v))} type="number" />
            <FI label="チェックイン日" value={form.check_in_date} onChange={set('check_in_date')} type="date" />
            <FI label="チェックアウト日" value={form.check_out_date} onChange={set('check_out_date')} type="date" />
            <FI label="宿泊人数" value={form.guests} onChange={v=>set('guests')(Number(v))} type="number" />
            <FI label="客室数" value={form.rooms} onChange={v=>set('rooms')(Number(v))} type="number" />
            <FI label="売上予定（円）" value={form.revenue} onChange={v=>set('revenue')(Number(v))} type="number" />
            <FS label="成果報酬率" value={form.commission_rate} onChange={set('commission_rate')} options={COMM_RATES} />
            <FS label="担当営業" value={form.source} onChange={set('source')} options={PERSONS} />
          </G2>
          <FT label="備考" value={form.notes} onChange={set('notes')} rows={2} />
        </Modal>
      )}
