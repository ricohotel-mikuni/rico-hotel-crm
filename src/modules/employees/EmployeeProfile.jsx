...
import { PageLoader, ErrorState, Btn, Badge, Empty, FV, G2, Toast } from '../../ui'
...
function TabPanel({ tabId, employeeId }) {
  ...
  const { data, loading } = useTable(table, q => q.select('*').eq('employee_id', employeeId).order('created_at', { ascending: false }))

  if (loading) return <PageLoader />
  if (data.length === 0) {
    return <Empty icon={TABS.find(t => t.id === tabId).icon} title="まだデータがありません" />
  }
  return (
    <div>
      {data.map(row => (
...
export default function EmployeeProfile() {
  ...
  if (!emp) return <HubShell><Empty icon="ti-user-off" title="社員が見つかりません" /></HubShell>
